...
function ItemOfTree({ item, onClick, isDisplayChildren,  className }) {
  ...
  const handleClick = (e, i, index) => { //свернуть/развернуть 
    e.stopPropagation();
    console.log(visibilityChildren)
    if (i['childNodes']) {
  ...
  const renderDisplayChildren =()=>{ 
    return (
    <>{childrenList && isDisplayChildren &&
      <ul>
        {
          childrenList.map((i, index) => (
            <ItemOfTree
              ...
              className={`${getClassName(index, i)} ${itemClasses[index]}`}
